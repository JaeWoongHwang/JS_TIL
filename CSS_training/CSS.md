...
- 다양한 요구사항에 대한 충족을 위해 특정 웹 페이지에 필요한 것보다 더 많은 코드를 포함되어 있을 수 있다. 이러한 경우를 일반적으로 '코드 블로트(Code Bloat)', 코드가 부풀려지는 현상이라 한다.
...
background-attachment 프로퍼티는 이미지를 한 곳에 고정시킬지, 아니면 사용자가 페이지를 스크롤하여 내려가거나 올라갈 때 이동할지 여부를 지정한다. 다음의 두가지 값 중에 하나를 설정할 수 있다.
...
6. 이미지의 alt 텍스트 : 검색 엔진은 alt 텍스트에 있는 이미지의 상세한 설명을 분석한다. alt 텍스트를 사용하여 검색 창에 이미지를 직접입력하여도 검색 결과에 이미지를 보여줄 수 있다.
7. 페이지 설명 : head 요소 내부에 name 값이 description 인 meta 태그를 지정한다. 그리고 content 값에 페이지의 설명을 작성한다. 설명이 브라우저 창에 나타나지는 않지만 검색 엔진의 결과 페이지에는 표시될 수 있다.



**오프 페이지(Off-Page)최적화**

해당 사이트를 링크하는 다른 사이트에 대한 분석은 온페이지 최적화만큼 중요하다. 검색 엔진은 사이트의 순위를 결정할 땓 해당 사이트를 링크한 다른 사이트의 개수도 고려한다.

검색 엔진은 특히 해당 사이트와 관련된 콘텐츠가 있는 사이트에 관심을 둔다. 예컨대 검색 엔진은 물고기의 미끼를 판매하는 사이트인 경우에 미용관련 사이트에서의 링크보다 낚시 커뮤니티에서의 링크가 관련성이 더 높다고 간주한다.

검색 엔진은 또한 a 태그 사이의 용어를 살펴본다. 단순히 웹사이트 주소나 '여기를 클릭하세요'라는 단어가 아니라 링크의 텍스트에 키워드가 포함돼 있으면 관련성이 더 높다고 간주한다.

사이트의 링크에 있는 단어는 사이트 링크를 제공하는 페이지의 텍스트에도 존재한다.



### 키워드와 구문을 식별하는 방법

>  **SEO 에 대한 고민을 시작하면 사이트에서 사용할 키워드를 결정하는 일이 가장 어려운 작업 중 하나가 될 수 있다. 사이트에 적합한 키워드와 구문을 식별하는 데 도움이 되는 여섯 단계는 다음과 같다.**



**브레인스톰**

사이트 검색을 위해 사라들이 구글에 입력할 만한 단어를 나열해 본다. 사이트에 관한 다양한 주제, 상품, 서비스를 포함해야 한다.

**구성**

키워드를 웹사이트의 카테고리나 섹션에 대한 각각의 목록으로 그룹화 한다.
대형 사이트의 경우 예컨대 애완 동물 사료 브랜드 같은 하위 카테고리를 만들어 더 세분화 할 수 있다.

**조사**

다음 도구는 키워드를 입력하면 고려할만한 추가적인 키워드를 제공한다.

- adwords.google.co.kr/select/KeywordToolExternal(사용시 내 검색어와 밀접하게 관련된 아이디어만 표시합니다를 체크한다.)
- www.wordtracker.com
- www.keyworddiscovery.com

이러한 도구가 추가적인 키워드를 제공하면 목록에 관련 옵션을 추가한다.

**비교**

사이트를 모든 키워드에 대해 검색 결과의 상단에 나타나게 할 수는 없다. 특히 경쟁자가 많은 주제에 대해선 더욱 그러하다.
이때 구글의 고급검색을 사용하여 웹 페이지의 제목만 검색해 본다. 이렇게 하면 페이지의 제목에 키워드가 있는 사이트가 얼마나 되는지 파악할 수 있다. 

**정제**

집중할 키워드가 무엇인지 선정해야 한다. 

**배치**

키워드 목록을 정제했으므로 가장 경쟁력 있는 키워드가 무엇인지, 가장 관련 있는 키워드가 무엇인지 파악할 수 있다. 따라서 지금부터는 각 페이지에서 사용할 키워드를 선정하는 작업을 시작해야 한다.

웹사이트의  각 페이지와 연관된 3~5개의 키워드나 구문을 선택하고 각 페이지의 키워드로 사용한다.



### 애널리틱스 : 방문자 분석

방문자 분석을 수행할 수 있는 가장 좋은 도구 중 하나는 구글 애널리틱스이다. 

구글 애널리틱스 서비스를 사용하기 위해 www.google.com/analytics에 가입한다.

**동작방식**

사용자가 페이지를 로드할 때마다 추적도구는 구글 서버로 데이터를 전송하고 적용한다. 그리고 구글은 사용자가 어떻게 사이트를 사용했는지 확인할 수 있도록 웹 기반의 인터페이스를 제공한다. 

**추적코드**

추적 코드는 추적하는 각 페이지에 대해 구글 애널리틱스가 제공한다. 추적 코드는 head 태그 바로 앞에 추가해야 한다. 하지만 코드가 웹 페이지의 모양에 영향을 미치지는 않는다.
...
호스팅 회사가 사이트 사용자에게 전달하는 데이터 양을 의미한다. 만일 10명의 사람이 동시에 사이트의 ㅗ든 페이지를 본다고 가정하면 사용하는 대역폭은 디스크 공간의 10배에 해당하는 양이 된다.



### Seventh Chapter Summary

- 검색 엔진 최적화는 검색 엔진 사용 시 방문자가 해당 사이트를 찾게 될 확률을 높여준다.
- 구글 애널리틱스 같ㅌ은 분석 도구를 통해 사이트 방문수, 방문자 유입 경로, 방문 시점등을 확인할 수 있다.
- 웹에 사이트를 올리려면 도메인 명과 웹 호스팅이 필요하다.
- FTP 프로그램을 사용하여 로컬 컴퓨터에서 웹 서버로 파일을 전송할 수 있다.
...
브라우저에 결과 페이지가 아니라 해당 코드가 보인다.

> **파일의 확장자가 .html  인지 확인한다.**

굵은체, 기울임꼴, 제목, 링크 텍스트가 생각보다 길게 보인다.
...
> **이미지를 올바른 포맷과 크기로 저장했는지 확인한다.**

테두리 이미지가 보이지 않는다.

> **해당 URL 확인과 함께 최신 버전으로 브라우저를 업데이트 한다.**

크기를 조정한 GIF 의 해상도가 떨어지거나 가장자리에 계단 현상이 발생한다.

> **이미지 편집 프로그램에서 색 공간(color space)을 확인한다. 색 공간은 색인 색상(indexed color)이 아니라 RGB이어야 한다.**



**텍스트**

올바른 폰트가 보이기 전에 텍스트가 잠깐 나타났다 사라진다. 

> **FOUC(Flash Of Unstyled Content)를 확인한다,**

일부 화면에서 텍스트가 크거나 작게 보인다.

> **화면 해상도의 차이로 발생한다. body요소에 크기가 설정돼 있는지 확인한다.**

em 단위를 사용하면 폰트 크기가 IE에서 올바르게 표현되지 않는다.

지정한 폰트가 일부 브라우저에서 보이지 않는다. 

> **컴퓨터에 폰트가 설치돼 있어야 한다.**
> **@font-face를 사용할 경우 여러 가지 포맷을 지정할 수 있다.**

폰트가 PC에서 계단 현상이 발생 한다.

> **일부 폰트는 안티 앨리어스를 지원하지 않는다. 다른 서체 또는 굵은체를 사용해 본다.**
